...
return (
  <div style={{ display: 'flex' }}>
    ...
    <div style={{ flex: 1, padding: '20px' }}>
      ...
      <h2 style={{ marginBottom: '10px', fontSize: '20px', fontFamily: 'Orbitron' }}>
        ...
        {themesData.map((theme, index) => (
          <div key={theme.id}>
            ...
            {selectedTheme && selectedTheme.id === theme.id && (
              <ul>
                {selectedThemeFinancialTerms.map((financialTerm, termIndex) => (
                  <div key={financialTerm.id}>
                    {/* Utilisez le lien correct avec l'ID du terme financier */}
                    <div
                 ...
                 fontFamily: 'Orbitron',
                        textDecoration: 'none',
                        background: '#F72585', // Ajoutez la couleur de fond dodgerblue ici
                        padding: '10px',
                        borderRadius: '5px',
                        marginBottom: '5px',
                      }}
                    ...
                    {/* Ajoutez une ligne horizontale sauf pour le dernier élément */}
                    {termIndex < selectedThemeFinancialTerms.length - 1 && <hr style={{ margin: '10px 0', border: '0.5px solid #fff' }} />}
                  </div>
                ))}
              </ul>
            )}
            {/* Ajoutez une ligne horizontale sauf pour le dernier élément */}
            {index < themesData.length - 1 && <hr style={{ margin: '10px 0', border: '0.5px solid #fff' }} />}
          </div>
        ))}
      </ul>
